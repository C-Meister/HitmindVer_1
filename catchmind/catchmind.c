...
if (on.eraser == true || click.eraser == true) { // eraser가 클릭 되어 있거나 eraser아이콘위에 마우스가 있으면
SDL_RenderRemoveEdge(Renderer, &Eraser);
RenderTexture(Renderer, EraTexture, &Eraser);// 렌더러에 저장하기
SDL_RenderDrawEdge(Renderer, &Eraser, click.eraser);
}
if (on.pencil == true || click.pencil == true) { // pencil이 클릭 되어 있거나 pencil아이콘위에 마우스가 있으면
SDL_RenderRemoveEdge(Renderer, &Pencil);
RenderTexture(Renderer, PenTexture, &Pencil);// 렌더러에 저장하기
SDL_RenderDrawEdge(Renderer, &Pencil, click.pencil);
}
if (on.new == true) {
...
if (click.eraser == true || click.pencil == true)
...
int main(int argc, char ** argv) {// 이 메인은 SDL.h에 선언된 메인함수로 우리가 흔히 쓰는 메인이 아님, 따라서 매개변수도 맞춰줘야함
...
else if(click.eraser == true) {// 지우개 경우
...
b = b + b / 5 * (alpha - 4);
// 폰트 출력 부분
if (click.pencil == true || click.eraser == true) {
SDL_SetRenderDrawColor(Renderer, 255, 255, 255, 0);// 흰색으로 정함
SDL_RenderFillRect(Renderer, &Font);// 폰트를 출력함. 근데 흰색이므로 지워주는 역할을 하게됨
SDL_SetRenderDrawColor(Renderer, r, g, b, 0);//색깔을 얻어옴
SDL_RenderFillRect(Renderer, &Font);// 폰트를 출력함
