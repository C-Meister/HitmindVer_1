...
int main(int argc, char **argv) //main함수 SDL에서는 인수와 리턴을 꼭 해줘야함 
{
	...
	char mysqlip[30] = "192.168.0.37";		//mysql ip 강당 ip = 192.168.0.37 상희ip입니다	지금 윈도우7버전 : 10.80.162.92 윈도우10버전 : 10.80.161.182
	...
	while (1) {								//로그인 반복문
		...
		if (mainchoose == 1) {				//main에서 첫번째를 고르면
			...
			while (1) {						//방 반복문
				...
				{
					if (connectroom[bangchoose - 2].people == 4)  //4명일
						chooseroomcount = -1;
...
void loadmysql(char mysqlip[])	//MYSQL 서버 불러오기
{
	...
	if (cons == NULL) //cons가 초기화를 못했으면
	{
		...
		printf("1");
	if (mysql_real_connect(cons, mysqlip, "root", "password", NULL, 0, NULL, 0) == NULL) //mysql서버로 연결
	{
		printf("\b실패... \n서버가 존재하지 않습니다.\n");
		fprintf(stderr, "%s\n", mysql_error(cons));
...
void recieve(void) { //서버에서 데이터 받아오는 쓰레드용 함수
	...
	while (1) {
		...
		if (recv(connect_sock, message, 45, 0) > 0) { //서버에서 데이터를 받아와 message변수에 저장
			...
			else if (strcmp(message, "choose 1") == 0)
			{
				score[0][1]++;
				CurrectHappen = true;
			}
			else if (strcmp(message, "choose 2") == 0)
			{
				score[1][1]++;
				CurrectHappen = true;

			}
			else if (strcmp(message, "choose 3") == 0)
			{
				score[2][1]++;
				CurrectHappen = true;

			}
			else if (strcmp(message, "choose 4") == 0)
			{
				score[3][1]++;
				CurrectHappen = true;

			}
			else if (strcmp(message, "pass 1") == 0)
			{
				turn = 1;
				PASS = true;

			}
			else if (strcmp(message, "pass 2") == 0)
			{
				turn = 2;
				PASS = true;

			}
			else if (strcmp(message, "pass 3") == 0)
			{
				turn = 3;
				PASS = true;

			}
			else if (strcmp(message, "pass 4") == 0)
			{
				turn = 4;
				PASS = true;
...
int sqllogin(void) {
	...
	while (1) {
		...
		if (user.id[0] == 0)
		{
			...
			mysql_query(cons, query);
			sql_result = mysql_store_result(cons);
			if (mysql_fetch_row(sql_result) == NULL)									//해 id가 없으면 
			{
				gotoxy(2, 3);
...
void Clnt_1(int v)
{
	...
	while (1) {
		if (recv(Sconnect_sock[v], message, 45, 0) > 0) {
			...
			else if (strcmp(message, "cont end") == 0)
			{
				RESET(message);
				fprintf(message, "cont %d end", v + 1);
				sendall(message, 5);

			}
			else if (strncmp(message, "pass ", 5) == 0) {
				sendall(message, 5);
			}
...
void SDL_FontUpdate(SDL_Renderer * Renderer, SDL_Rect* Font, SDL_Rect Track, float strong, int r, int g, int b) {
	if (clicks.pencil == true) {
		...
		Font->y = Track.y - 47 - strong / 2;// 오른쪽 꼭짓점 좌표를 다시설정
...
void SDL_RenderUpdate(SDL_Renderer* Renderer, SDL_Renderer* Renderer2, SDL_Renderer* Renderer3, SDL_Texture* TraTexture, SDL_Texture* BoxTexture, SDL_Texture* EraTexture, SDL_Texture* PenTexture, SDL_Texture* NewTexture, SDL_Texture* ChaTexture, SDL_Texture* InpTexture, SDL_Texture* MagTexture, SDL_Texture* PasTexture, SDL_Texture* RecTexture, SDL_Rect *Track, SDL_Rect *Box, SDL_Rect *Eraser, SDL_Rect *Pencil, SDL_Rect *New, SDL_Rect *Font, SDL_Rect* Chat, SDL_Rect *InputT, SDL_Rect *Magnifying, SDL_Rect *Pass, SDL_Rect *Recycle, TTF_Font* Fonts, wchar_t* inputText, float *strong, int r, int g, int b) {
	...
	if (on.pass == true) {
		RenderTexture(Renderer, PasTexture, Pass);// 렌더러에 저장하기
		SDL_RenderDrawEdge(Renderer, Pass, 0);
	}
	if (on.magnifying == true) {
		RenderTexture(Renderer, MagTexture, Magnifying);// 렌더러에 저장하기
		SDL_RenderDrawEdge(Renderer, Magnifying, 0);
	}
	if (on.recycle == true) {
		RenderTexture(Renderer, RecTexture, Recycle);// 렌더러에 저장하기
...
//	const Uint8 * keystate;// Key 상태 배열을 받기 위한 포인터 선언
	...
	SDL_Rect Fonts = { Track.x - strong / 2 + 35 ,Track.y - strong / 2 - 45,strong,strong };// 색깔, 굵기등을 보여주기 위한 사각형 변수 선언
				...
				if (writemode == true) {
					...
					if ((event.button.x >= Magnifying.x - 10 && event.button.x <= Magnifying.x + Magnifying.w + 10) && (event.button.y >= Magnifying.y - 10 && event.button.y <= Magnifying.y + Magnifying.h + 10)) {
						...
						if (turn != myownnumber) {
							...
							LeaveCriticalSection(&cs);
							RESET(query);
							sprintf(query, "%d글자 입니다", strlen(topics[turn - 1]) / 2);
							han2unicode(query, unicode);
		...
		if (CHATHAPPEN == true) {
			...
			for (l = 0; l < 15; l++) {
				if (chatquery[(int)l][0] != 0) {
					...
					TTF_DrawText(Renderer, Font, unicode, 10, 295 + 25 * l);		//최근 15개의 채팅을 불러옴
	...
	while (!quit) {// quit가 true가 아닐때 동안 무한반복
		...
		if ((clock() - firstclock) / 1000 > first && vote == false && MAKETOPIC == false)
		{
			...
			if (first == connectroom[CHOOSEROOM].time + 1)				//1초마다 화면을 초기화하여서 올림
			{
				...
				//mode가 컨테스트이면 서로의 화면을 4등분해서 보여줌
				SDL_SetRenderDrawColor(Renderer2, 255, 255, 255, 0);
				SDL_RenderClear(Renderer2);
				for (i = 1; i <= 4; i++) {
					if (status[(int)i - 1] != 0)
		...
		if ((clock() - firstclock) / 1000 > first && vote == true && MAKETOPIC == false)
		{
			...
			if (first > 20)
			{
				...
				{
					RESET(chatquery);
					strcpy(chatquery[13], "투표를 가장 많이 받으셧군요.");
					strcpy(chatquery[14], "/topic [주제] 로 주제를 선정해 주세요");
					CHATHAPPEN = true;
					firsttopic = true;
				}
				SDL_DestroyRenderer(Renderer2);
		...
		if ((clock() - firstclock) / 1000 > first && MAKETOPIC == true)
		{
			first++;
			if (first == 21 && firsttopic == true)
			{
				firsttopic = false;
				EnterCriticalSection(&cs);
				mysql_query(cons, "select top from catchmind.topic order by rand() limit 1");
			...
			SDL_SetRenderDrawColor(Renderer, 255, 255, 255, 0);// 색깔을 흰색으로 설정해야함 그래야 지우개 역할을 하므로
			SDL_RenderFillRect(Renderer, &Timer2);// 지우개같이 흰색으로 칠함
			sprintf(query, "주제선정 %d초 남음", 21 - first);
			han2unicode(query, unicode);
			TTF_DrawText(Renderer, Font, unicode, 150, 150);
			happen = true;
		}
		...
		if (SDL_PollEvent(&event)) {//이벤트가 있으면 if문 실행
				...
				if (event.key.keysym.sym == SDLK_RETURN) {
					if (hangeulinput == true && enter == false)
						...
						else if (strcmp(euckr, "/help") == 0)
						{
							...
							RESET(chatquery);
							//		EnterCriticalSection(&cs);
							strcpy(chatquery[9], "[1] /help : 도움말을 표시합니다.");

							strcpy(chatquery[10], "[2] /clear : 채팅창을 초기화합니다.");

							strcpy(chatquery[11], "[3] /capture [파일명] : 현재 그림을 캡쳐.");

							strcpy(chatquery[12], "[4] /stopmusic : 현재 음악을 멈춥니다.");

							strcpy(chatquery[13], "[5] /startmusic : 현재 음악을 재실행합니다.");

							strcpy(chatquery[14], "[6] /topic [주제]: 주제를 정합니다. 투표 1등만 가능");
							//		LeaveCriticalSection(&cs);
							CHATHAPPEN = true;
						...
						else if (strncmp(euckr, "/topic ", 7) == 0)
						{
							...
							{
								RESET(chatquery);
								strcpy(chatquery[14], "주제 선정 권한이 없습니다.");
							}
							else
							{
								...
								EnterCriticalSection(&cs);
								sprintf(euckr, "insert into catchmind.chating (name, mean, room) values ('[주제]','%s님이 %s로 설정하였습니다.', '%s')",username, query, connectroom[CHOOSEROOM].ip);
			...
			case SDL_MOUSEBUTTONDOWN:
				if (vote == true) {
					if (canvote == true && (event.button.x >= 0 && event.button.x <= (1920 - 1310 / 4 - 10) / 2) && (event.button.y >= 0 && event.button.y <= (1080 - 900 / 4 - 10) / 2) && status[0] != 0) {
						canvote = false;
						send(connect_sock, "choose 1", 35, 0);
					}
					if (canvote == true && (event.button.x > (1920 - 1310 / 4 - 10) / 2 && event.button.x <= 1920 - 1310 / 4 - 10) && (event.button.y >= 0 && event.button.y <= (1080 - 900 / 4 - 10) / 2) && status[1] != 0) {
						canvote = false;
						send(connect_sock, "choose 2", 35, 0);
					}
					if (canvote == true && (event.button.x >= 0 && event.button.x <= (1920 - 1310 / 4 - 10) / 2) && (event.button.y > (1080 - 900 / 4 - 10) / 2 && event.button.y <= 1080 - 900 / 4 - 10) && status[2] != 0) {
						canvote = false;
						send(connect_sock, "choose 3", 35, 0);
					}
					if (canvote == true && (event.button.x > (1920 - 1310 / 4 - 10) / 2 && event.button.x <= 1920 - 1310 / 4 - 10) && (event.button.y > (1080 - 900 / 4 - 10) / 2 && event.button.y <= 1080 - 900 / 4 - 10) && status[3] != 0) {
						canvote = false;
						send(connect_sock, "choose 4", 35, 0);
					}
				}
		...
		if (CHATHAPPEN == true) {
			...
			for (l = 0; l < 15; l++) {
				if (chatquery[(int)l][0] != 0) {
					han2unicode(chatquery[(int)l], unicode);
					TTF_DrawText(Renderer, Font, unicode, 10, 290 + 25 * l);		//최근 15개의 채팅을 불러옴
					ZeroMemory(unicode, sizeof(unicode));// 추가
				}
			}
			sprintf(query, "문제 %d/%d", ee + 1, connectroom[CHOOSEROOM].question);
			han2unicode(query, unicode);
